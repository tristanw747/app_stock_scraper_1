...
export let sharedFinalStocksNoFormat= [];
async function afuncMatchFilter(data) {
  [sharedFinalStocksArray, sharedFinalStocksNoFormat, sharedFinalArticlesArray, sharedFinalIndexArray] = [[], [], [], []];
  ...
  for (let i = 0; i < inputDataArray.length; i++) {
    ...
    if (tempMatchedStock) {
      ...
      if (!sharedFinalStocksArray.includes('<br/>' + '* ' + tempMatchedStock[0])) {
        ...
        sharedFinalStocksNoFormat.push(tempMatchedStock[0]);///////////////
      }
...
//////////////////////////////////////////////////
let responseJsonStockStatsGoogle;
async function afuncStockStatsGoogle() {
  let responseStockStatsGoogle = await fetch("/StockStatsGoogle");
  responseJsonStockStatsGoogle = await responseStockStatsGoogle.json().catch(err => console.log("Notice from Developer: Incorrect server page url, check url directory. ", err));
  if (responseJsonStockStatsGoogle.product1[0] === undefined) {
    document.querySelector("#StockStatsGoogleSendtoHTML1").innerHTML = "Error! Scraping Search Tag Not Found!"
  } else {

    document.querySelector("#StockStatsGoogleSendtoHTML1").innerHTML = responseJsonStockStatsGoogle.product1
    document.querySelector("#StockStatsGoogleSendtoHTML2").innerHTML = responseJsonStockStatsGoogle.product2
  }
}
